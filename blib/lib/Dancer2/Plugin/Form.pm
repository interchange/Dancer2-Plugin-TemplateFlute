...
register form => sub {    
    ...
    $object = Dancer2::Plugin::Form->new($dsl, name => $name, @_);
...
C<Dancer2::Plugin::Form> is used for forms with the L<Dancer2::Template::TemplateFlute>
templating engine.    
...
Creates C<Dancer2::Plugin::Form> object.
    
=cut

sub new {    
    my ($class, $dsl, $self, %params);
    $class = shift;
    $dsl = shift;
...
sub valid {
    ...
    if (defined $valid) {
	    $self->dsl->debug("Setting valid for $self->{name} to $valid.");
        $self->{valid} = $valid;
...
sub failure {
    ...
    $dsl->app->session->write(
        form_errors => '<ul>' . join('', map {"<li>$_</li>"} CORE::values %{$args{errors} || {}}) . '</ul>');
